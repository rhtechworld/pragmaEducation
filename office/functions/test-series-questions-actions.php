...
if(isset($_POST['updateQuestionNow']))
{
    ...
    $inputOfQtnSecduleId = mysqli_real_escape_string($conn, $_POST['inputOfQtnSecduleId']);

    //update in database
    $updateDataInDatabase = mysqli_query($conn,"UPDATE test_series_qtns SET qtn='$qtnInput',
    ...
    {
        echo '<script>alert("Updated Success, Refreshing...")</script>';
        header('refresh:0;url=test-series-questions?ts_id='.$ts_id.'&sc_id='.$inputOfQtnSecduleId.'');
    }
...
if(isset($_POST['deleteQuestionNow']))
{
    ...
    //update in database
    $updateDataInDatabase = mysqli_query($conn,"UPDATE test_series_qtns SET isDeleted='1', date='$lastUpdated' WHERE qtn_id='$inputOfQtnId'");

    echo '<script>alert("Deleted Success, Refreshing...")</script>';
    header('refresh:0;url=test-series-questions?ts_id='.$inputOfQuizId.'&sc_id='.$inputOfQtnSecduleId.'');
}
...
if(isset($_POST['addNewQuestionNowViaAccess']))
{
    ...
    $insertIntoDatabase = mysqli_query($conn,"INSERT INTO test_series_qtns(ts_id, sc_id, ts_type, qtn_no, qtn_id, qtn, choice_one, choice_two, choice_three, choice_four, qtn_ans, status, date, isDeleted)
    VALUES('$inputOfQuizId','$inputOfQtnSecduleId','$inputOfQuizType','$inputOfQtnNo','$inputOfQtnId','$qtnInput','$optionOneInput','$optionTwoInput','$optionThreeInput','$optionFourInput','$thisAnswerInput','$qtnquizStatusInput','$lastUpdated','0')");

    echo '<script>alert("Question : '.$inputOfQtnNo.' Added Success, Moving to Questions List...")</script>';
    header('refresh:0;url=test-series-questions?ts_id='.$inputOfQuizId.'&sc_id='.$inputOfQtnSecduleId.'');
}

if(isset($_POST['updatePassmarknowOnThisAction']))
{
    $passMarkNumberNow = mysqli_real_escape_string($conn, $_POST['passMarkNumberNow']);

    //update in database
    $updateDataInDatabase = mysqli_query($conn,"UPDATE test_series_schedule SET sc_pass_mark='$passMarkNumberNow', last_updated='$lastUpdated' WHERE sc_id='$sc_id'");
