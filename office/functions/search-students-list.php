...
if(isset($_POST['searchStudentAction']))
{

    $studentSearch = mysqli_real_escape_string($conn,$_POST['searchStudent']);

    //get all Students
    $getAllStudents = mysqli_query($conn,"SELECT * FROM students WHERE student_id='$studentSearch' OR student_email='$studentSearch' OR student_number='$studentSearch' AND isDeleted='0' ORDER BY id DESC");
    $getCOuntOfStudents = mysqli_num_rows($getAllStudents);

    if($getCOuntOfStudents == 0)
    {
        echo '<b class="mb-2 text-danger text-center">No Search Results....</b>';
    }
    else
    {
        $sl = 1;
