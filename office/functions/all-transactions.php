...
{
    $filterDateIs = mysqli_real_escape_string($conn, $_GET['filter']);
    $getTxnsOnDashboard = mysqli_query($conn,"SELECT * FROM student_txns WHERE isDeleted='0' AND date='$todayDate' ORDER BY id DESC");
    $getCOuntOngetTxnsOnDashboard = mysqli_num_rows($getTxnsOnDashboard);
}
else
{
    $getTxnsOnDashboard = mysqli_query($conn,"SELECT * FROM student_txns WHERE isDeleted='0' ORDER BY id DESC");
    $getCOuntOngetTxnsOnDashboard = mysqli_num_rows($getTxnsOnDashboard);
}
